...
const createBookingRequest = (athlete,unitAccountId,  amount, description,receiverAccountType ) => unit.getAthleteUnitAccountById(unitAccountId).then(res => (res.attributes.available >= amount) ? 
...
module.exports.podAllocation = async (event) => {
  ...
  return tpc.getAthlete(axios, athleteId).then(res => createBookingRequest(res, find(propEq('podName', 'SPENDING'))(res?.accounts?.items).unitAccountId, amount, description, receiverAccountType, receiverUnitAccountId, idempotencyKey));
}