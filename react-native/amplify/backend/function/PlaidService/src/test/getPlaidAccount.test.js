...
describe('getPlaidAccount', () => {
    ...
    beforeEach(() => {
        ...
        getPlaidAccountStub = sinon.stub(plaid, "getPlaidAccount");
      });
    
      afterEach(() => {
        getAthleteStub.restore();
        getPlaidAccountStub.restore();
    });
    ...
    describe("before doing any work", () => {
      ...
      it("should return failed, if plaid call is not successful", async () => {
        ...
        try {
          await getPlaidAccountStub(event);
        } catch {} finally {
    ...
    it("should return generated link for the athlete if plaid return successful", async () => {
        ...
        await getPlaidAccount(event);
        sinon.assert.equal(persistChallengeStub.callCount, 1);
      });
});